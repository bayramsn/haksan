...
import { LASER_POWERS, laserSelectionSchema, type LaserSelection, type LaserSpec, type LaserTechnicalConfiguration } from '@haksan/shared';
import { AlertCircle, ArrowDown, ArrowUp, FileSpreadsheet, Loader2, Plus, RotateCcw, Trash2 } from 'lucide-react';
...
export interface LaserConfigurationEditorProps {
  ...
  disabled?: boolean;
  /** Use the common technical workbook for field editing. */
...
function LaserEditor({ divisionId, brandId, value, onChange, initialProductTypeCode, onSelectionChange, disabled, selectionOnly, scope }: LaserConfigurationEditorProps & { scope: LaserDraftScope }) {
  const id = useId();
  ...
  const saveDraft = (next: LaserTechnicalConfiguration) => {
    publish(next);
  ...
  const updateSpec = (index: number, nextValue: string, restoreSource = false) => {
    ...
    saveDraft({ ...configuration, specs: configuration.specs.map((spec, i) => i === index ? (restoreSource ? resetLaserSpec(spec) : editLaserSpec(spec, nextValue)) : spec) });
  };
  const updateField = (index: number, patch: Partial<LaserSpec>) => {
    if (!configuration) return;
    saveDraft({ ...configuration, specs: configuration.specs.map((spec, i) => i === index ? {
      ...spec, sourceValue: spec.sourceValue ?? (spec.isManual ? '' : spec.value), sourceUnit: spec.sourceUnit ?? spec.unit,
      sourceGroupCode: spec.sourceGroupCode ?? spec.groupCode, ...patch, isManual: true,
    } : spec) });
  };
  const moveField = (index: number, step: number) => {
    if (!configuration || index + step < 0 || index + step >= configuration.specs.length) return;
    const specs = [...configuration.specs];
    [specs[index], specs[index + step]] = [specs[index + step], specs[index]];
    saveDraft({ ...configuration, specs });
  };
  const removeField = (index: number) => {
    if (!configuration || configuration.specs[index].key === 'Lazer Gücü') return;
    saveDraft({ ...configuration, specs: configuration.specs.filter((_, i) => i !== index) });
  };
  const addField = () => {
    if (!configuration || configuration.specs.length >= 150) return;
    let number = 1;
    while (configuration.specs.some((spec) => spec.key === `Yeni teknik bilgi ${number}`)) number++;
    saveDraft({ ...configuration, specs: [...configuration.specs, { key: `Yeni teknik bilgi ${number}`, value: '', unit: '', groupCode: 'GENEL', sourceValue: '', isManual: true }] });
  };
  ...
  return (
    <section className="space-y-4" aria-label="Lazer teknik yapılandırması">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <div className="space-y-1.5"><Label htmlFor={`${id}-series`}>3. Ürün serisi tipi</Label><select id={`${id}-series`} className={selectClass} disabled={inputDisabled || !selection.productTypeCode || loadingOptions || !options} value={selection.series ?? ''} onChange={(event) => change('series', event.target.value)}><option value="">Seri seçin</option>{Array.from(new Set((options?.models ?? []).filter((model) => model.productTypeCode === selection.productTypeCode).map((model) => model.series))).map((series) => <option key={series} value={series}>{series} Serisi</option>)}</select></div>
        <div className="space-y-1.5"><Label htmlFor={`${id}-cabin`}>4. Kabin tipi</Label><select id={`${id}-cabin`} className={selectClass} aria-describedby={`${id}-cabin-hint`} disabled={inputDisabled || !selection.series} value={selection.cabinType ?? ''} onChange={(event) => change('cabinType', event.target.value)}><option value="">Kabin seçin</option><option value="open">Açık</option><option value="closed">Kapalı</option></select><p id={`${id}-cabin-hint`} className="text-xs text-slate-500">{isTube ? 'Kesim bölgesi koruması; boru besleme alanını kapsamaz.' : selection.series === 'F' ? 'Katalog: açık kabin, tek tabla.' : selection.series === 'PG' ? 'Katalog: kapalı kabin, değişimli çift tabla.' : models.length && models.every((model) => model.standardCabin === 'open') ? 'Kaynak: açık kabin.' : models.length && models.every((model) => model.standardCabin === 'closed') ? 'Kaynak: kapalı kabin.' : 'Kaynakta kabin eşlemesi doğrulanmayan modellerin ilgili alanları elle tamamlanır.'}</p></div>
        <div className="space-y-1.5"><Label htmlFor={`${id}-power`}>5. Rezonatör gücü</Label><select id={`${id}-power`} className={selectClass} disabled={inputDisabled || !selection.cabinType} value={selection.powerKw ?? ''} onChange={(event) => change('powerKw', event.target.value)}><option value="">Güç seçin</option>{(options?.powerOptions ?? LASER_POWERS).map((power) => <option key={power} value={power}>{String(power).replace('.', ',')} kW</option>)}</select></div>
      ...
      {(!divisionId || !brandId) && <p className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Lazer teknik bilgileri için Sac İşleme bölümünü ve teknik kataloğu bağlı markayı seçin.</p>}
      ...
      {configuration && <>
        ...
        {!selectionOnly && <><div className="flex items-center justify-between gap-2"><p className="text-xs text-slate-500">Alan adı, değer, birim ve grubu düzenleyebilir; alan ekleyip kaldırabilirsiniz.</p><Button type="button" variant="outline" size="sm" disabled={disabled || loading || configuration.specs.length >= 150} onClick={addField}><Plus className="mr-1 size-4" />Alan ekle</Button></div><div className="max-h-[38rem] overflow-auto rounded-lg border border-slate-200">
          <table className="w-full min-w-[660px] border-collapse text-sm">
            ...
            <thead className="sticky top-0 z-10 bg-slate-100 text-left text-xs text-slate-700"><tr><th scope="col" className="px-3 py-3">Teknik bilgi</th><th scope="col" className="px-3 py-3">Değer</th><th scope="col" className="px-3 py-3">Birim</th><th scope="col" className="px-3 py-3">Grup</th><th scope="col" className="px-3 py-3">Kaynak / düzenleme</th><th scope="col" className="px-3 py-3">Sıra / kaldır</th></tr></thead>
            <tbody>{configuration.specs.map((spec, index) => <tr key={index} className="border-t border-slate-100 align-top">
              <th scope="row" className="min-w-44 px-3 py-3 text-left font-medium text-slate-800"><label className="sr-only" htmlFor={`${id}-spec-${index}`}>{spec.key}</label><Input value={spec.key} aria-label={`${index + 1}. alan adı`} readOnly={spec.key === 'Lazer Gücü'} disabled={disabled || loading} maxLength={255} onChange={(event) => updateField(index, { key: event.target.value })} /></th>
              ...
              <td className="min-w-24 px-3 py-2"><Input value={spec.unit ?? ''} aria-label={`${spec.key} birimi`} readOnly={spec.key === 'Lazer Gücü'} disabled={disabled || loading} maxLength={64} onChange={(event) => updateField(index, { unit: event.target.value })} /></td><td className="min-w-28 px-3 py-2"><Input value={spec.groupCode ?? ''} aria-label={`${spec.key} grubu`} disabled={disabled || loading} maxLength={64} onChange={(event) => updateField(index, { groupCode: event.target.value })} /></td>
              ...
              <td className="px-3 py-2"><div className="flex gap-1"><Button type="button" variant="ghost" size="icon" disabled={disabled || loading || index === 0} aria-label={`${spec.key} yukarı taşı`} onClick={() => moveField(index, -1)}><ArrowUp className="size-4" /></Button><Button type="button" variant="ghost" size="icon" disabled={disabled || loading || index === configuration.specs.length - 1} aria-label={`${spec.key} aşağı taşı`} onClick={() => moveField(index, 1)}><ArrowDown className="size-4" /></Button><Button type="button" variant="ghost" size="icon" disabled={disabled || loading || spec.key === 'Lazer Gücü'} aria-label={`${spec.key} kaldır`} onClick={() => removeField(index)}><Trash2 className="size-4" /></Button></div></td>
            </tr>)}</tbody>
          </table>
        </div></>}
      </>}
