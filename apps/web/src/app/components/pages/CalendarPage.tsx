...
const TYPE_STYLES: Record<CalendarEventType, string> = {
  customer_visit: 'border-emerald-300 bg-emerald-50 text-emerald-950',
  meeting: 'border-blue-300 bg-blue-50 text-blue-950',
  call: 'border-amber-300 bg-amber-50 text-amber-950',
  task: 'border-rose-300 bg-rose-50 text-rose-950',
  other: 'border-slate-300 bg-slate-50 text-slate-900',
};
...
function ImportRow({ event }: { event: CalendarImportEvent }) {
  ...
  return <div className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-muted/40">
    ...
    {!event.inWindow && <Badge variant="outline" className="shrink-0 text-amber-600">Pencere dışı</Badge>}
  </div>;
}
