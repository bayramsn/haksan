...
describe('HEXLASER sellable product variants', () => {
  it('creates one unique product for every model, cabin and power combination', () => {
    const variants = hexlaserCuttingVariants();
    expect(variants).toHaveLength(LASER_MODELS.length * 2 * 7);
  ...
  it('keeps PG3015 cabin and resonator powers as separate products', () => {
    ...
    expect(variants.map((variant) => variant.modelCode)).toEqual([
      'PG3015-KAPALI-1.5KW',
      'PG3015-KAPALI-3KW',
    ]);
    expect(variants.map((variant) => variant.fullName)).toEqual([
      'PG3015 Kapalı Kabin 1.5 kW Sac Lazer Kesim',
      'PG3015 Kapalı Kabin 3 kW Sac Lazer Kesim',
    ]);
    for (const variant of variants) {
  ...
  it('uses the catalog cabin and first supported power when migrating an old model product', () => {
    ...
    expect(hexlaserVariantCode(selection)).toBe('PG3015-KAPALI-1.5KW');
    expect(hexlaserVariantFullName(selection)).toBe('PG3015 Kapalı Kabin 1.5 kW Sac Lazer Kesim');
  });
