...
export const quoteService = {
   ...
   */
  downloadPdf: (id: string) => openPdfUrl(`/quotes/${id}/generate-pdf`),
  openPdf: (id: string) => openPdfUrl(`/quotes/${id}/generate-pdf`),
};
...
async function uploadViaSignedUrl(upload: SignedUploadResponse, file: Blob | ArrayBuffer, mimeType: string): Promise<void> {
...
async function uploadViaApi(fileId: string, file: Blob | ArrayBuffer, mimeType: string): Promise<void> {
  ...
  if (activeDepartment) headers['X-Active-Department'] = activeDepartment;

  const res = await fetch(`${getApiBaseUrl()}/files/${encodeURIComponent(fileId)}/content`, {
    method: 'PUT',
    headers,
    credentials: 'include',
    body: file,
...
export const fileService = {
  ...
  uploadBinary: async (upload: SignedUploadResponse, file: Blob | ArrayBuffer, mimeType: string) => {
    try {
...
export const adminService = {
  ...
  deleteUser: (id: string) => api.delete<any>(`/users/${id}`),
